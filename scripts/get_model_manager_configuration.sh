...
echo "Retrieving Model Manager Configuration"
modelManagerConfiguration=$(az keyvault secret show --vault-name $keyVaultName --name $secretName --query value -o tsv | tr -d '\r')

echo $modelManagerConfiguration > "Model-Manager-$deviceName-Configuration.json"
